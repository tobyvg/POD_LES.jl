...
function time_step(input,mesh,t,dt,rhs;other_arguments = 0,method = "RK4")
    if method == "RK4"
        ...
        k2 = rhs(input + Float32(dt/2)*k1,mesh,t .+ dt/2,other_arguments = other_arguments)

 
        #k1 = rhs(input,mesh,t,other_arguments = other_arguments)

        k3 = rhs(input + Float32(dt/2)*k2,mesh,t .+ dt/2,other_arguments = other_arguments)
        k4 = rhs(input + Float32(dt)*k3,mesh,t .+ dt,other_arguments = other_arguments)

        

        return Float32(1/6*dt)*(k1 + 2*k2 + 2*k3 + k4)
...
function simulate(input0,mesh,dt,t_start,t_end,rhs,time_step_function;save_every = 1,other_arguments = 0,pre_allocate = false, GC_every = 100)
    ...
    t = stop_gradient() do
        cu(ones(size(output_t)[1:end-2]...,size(input0)[end])) .* reshape(t_start,(size(output_t)[1:end-2]...,prod(size(t_start))))
    end
